...
/*
  Bitstream — Product Flow Visualization
  Ported from ACE Protocol landing page animation.
  ...
  Pure CSS + framer-motion. No Three.js.
*/
...
function StepConnect({ active }: { active: boolean }) {
  ...
  const txns = [
    { hash: 'a3f1…8d2c', amount: '-0.004 BTC', label: 'Mezo payment',    color: '#F7931A' },
    { hash: 'b7e9…1a4f', amount: '-142 MUSD',  label: 'AWS infra bill',  color: '#f59e0b' },
    { hash: 'c1d8…9e3b', amount: '+0.12 BTC',  label: 'Revenue inflow',  color: '#10d98c' },
    { hash: 'd4a2…7f6c', amount: '-500 MUSD',  label: 'Payroll cycle',   color: '#c084fc' },
  ];
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <motion.div
        ...
        style={{
          background: 'linear-gradient(135deg, rgba(247,147,26,0.10) 0%, rgba(16,217,140,0.04) 100%)',
      ...
      >
        ...
        {phase === 1 && (
          <motion.div
            ...
            style={{ background: 'linear-gradient(90deg, transparent, rgba(247,147,26,0.8), transparent)' }}
          />
        )}

        <div className="flex items-center gap-3 mb-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl flex-shrink-0"
            style={{ background: 'rgba(247,147,26,0.14)', border: '1px solid rgba(247,147,26,0.28)' }}>
            {/* BTC icon */}
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              <path d="M10.5 7c.5-.5.5-1.5-.5-2H6v2.5h4c.5 0 .5-.5.5-.5zM10 9.5H6V12h4c1 0 1.5-1 .5-1.5-.5-.5-1-1-.5-1z" fill="#F7931A"/>
              <path d="M8 2v1M8 13v1M6 2.5v1M10 2.5v1M6 12.5v1M10 12.5v1" stroke="#F7931A" strokeWidth="1.2" strokeLinecap="round"/>
            </svg>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em]" style={{ color: '#F7931A' }}>
              Bitcoin Treasury
            </p>
            <p className="text-[11px] font-mono mt-0.5 truncate" style={{ color: 'rgba(255,255,255,0.4)' }}>
              bc1q…m4qz · Mezo
            </p>
          </div>
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <motion.div
              ...
              style={{ background: '#10d98c', boxShadow: '0 0 5px #10d98c' }}
            />
            <span className="text-[9px] font-semibold" style={{ color: '#10d98c' }}>LIVE</span>
          </div>
        </div>
        ...
        <AnimatePresence>
          {phase >= 2 && (
            ...
            >
              <div className="flex items-end gap-4 pt-2 pb-1 border-t" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
                <div>
                  <p className="text-[8.5px] uppercase tracking-[0.16em]" style={{ color: 'rgba(255,255,255,0.3)' }}>
                    Treasury Balance
                  </p>
                  <p className="text-[22px] font-mono font-600 leading-none mt-1"
                    style={{ color: '#edf2ff', letterSpacing: '-0.04em' }}>
                    2.847{' '}
                    <span className="text-[13px]" style={{ color: 'rgba(255,255,255,0.38)' }}>BTC</span>
                  </p>
                </div>
                <div className="mb-0.5 ml-auto text-right">
                  <p className="text-[8.5px] uppercase tracking-[0.16em]" style={{ color: 'rgba(255,255,255,0.3)' }}>MUSD</p>
                  <p className="text-[15px] font-mono font-500 leading-none mt-0.5" style={{ color: '#F7931A', letterSpacing: '-0.03em' }}>
                    48,291
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-1.5">
              {txns.map((tx, i) => (
                ...
                >
                  ...
                  <span className="text-[11px] flex-1 truncate" style={{ color: 'rgba(255,255,255,0.52)' }}>{tx.label}</span>
                  <span className="text-[11px] font-mono font-500 flex-shrink-0" style={{ color: tx.color }}>{tx.amount}</span>
                  <span className="text-[9px] font-mono flex-shrink-0" style={{ color: 'rgba(255,255,255,0.18)' }}>{tx.hash}</span>
                </motion.div>
              ))}
            </div>
          </motion.div>
...
function StepDetect({ active }: { active: boolean }) {
  ...
  const patterns = [
    { label: 'Payroll Cycle',       frequency: 'Monthly · ~1st',     amount: '~500 MUSD',    confidence: 94, color: '#c084fc' },
    { label: 'Infrastructure Bill', frequency: 'Monthly · ~15th',    amount: '~142 MUSD',    confidence: 88, color: '#f59e0b' },
    { label: 'BTC Collateral Top-up', frequency: 'Weekly · variable',amount: '~0.04 BTC',    confidence: 76, color: '#F7931A' },
    { label: 'Revenue Inflow',      frequency: 'Irregular · detected',amount: '+0.12 BTC avg',confidence: 81, color: '#10d98c' },
  ];
  ...
  return (
    <div className="flex flex-col gap-3">
      <motion.div
        ...
        style={{ background: 'rgba(247,147,26,0.07)', border: '1px solid rgba(247,147,26,0.18)' }}
      >
        <motion.div
        ...
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            <circle cx="8" cy="8" r="6" stroke="#F7931A" strokeWidth="1.5" strokeDasharray="3 2" />
            <circle cx="8" cy="8" r="2" fill="#F7931A" />
          </svg>
        </motion.div>
        <p className="text-[11px]" style={{ color: '#F7931A' }}>
          AI analyzing <span className="font-mono font-500">847</span> BTC transactions
        </p>
      </motion.div>
            ...
            {revealed > i && (
              ...
              >
                <div className="flex items-center justify-between gap-2 mb-2">
                  <div>
                    <p className="text-[12px] font-semibold leading-none" style={{ color: 'rgba(255,255,255,0.88)' }}>{p.label}</p>
                    <p className="text-[9.5px] mt-0.5" style={{ color: 'rgba(255,255,255,0.3)' }}>{p.frequency}</p>
                  </div>
                  <p className="text-[12px] font-mono font-500 flex-shrink-0" style={{ color: p.color }}>{p.amount}</p>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] font-mono flex-shrink-0" style={{ color: p.color }}>{p.confidence}%</span>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
...
function StepPlan({ active }: { active: boolean }) {
  ...
  const payments = [
    { id: 'p1', label: 'Payroll · Team',       amount: '500 MUSD', due: 'in 3 days',  status: 'ready',  color: '#c084fc' },
    { id: 'p2', label: 'AWS / Infrastructure', amount: '142 MUSD', due: 'in 11 days', status: 'queued', color: '#f59e0b' },
    { id: 'p3', label: 'x402 API bill',        amount: '0.004 BTC',due: 'in 5 days',  status: 'queued', color: '#F7931A' },
    { id: 'p4', label: 'MUSD top-up',          amount: '0.02 BTC', due: 'in 18 days', status: 'queued', color: '#10d98c' },
  ];

  const statusColors: Record<string, string> = { ready: '#10d98c', queued: '#F7931A', executing: '#c084fc', done: '#3a4a6a' };
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-1 flex items-center gap-1.5">
                ...
                <span className="text-[9px] font-mono" style={{ color: '#10d98c' }}>2.847 BTC</span>
              </div>
            </div>
          </motion.div>
        )}
            ...
            <div className="space-y-1.5">
              {payments.map((p, i) => (
                ...
                >
                  ...
                  <span className="text-[11px] font-mono font-500 flex-shrink-0" style={{ color: p.color }}>{p.amount}</span>
                  <span className="text-[8px] font-semibold uppercase px-1.5 py-0.5 rounded-full flex-shrink-0"
                    style={{ background: `${statusColors[p.status]}14`, color: statusColors[p.status], border: `1px solid ${statusColors[p.status]}28` }}>
                    {p.status}
                  </span>
                </motion.div>
              ))}
...
function StepExecute({ active }: { active: boolean }) {
  ...
  const policies = [
    { label: 'Spending cap',          value: '2,500 MUSD / mo' },
    { label: 'Destination whitelist', value: '4 addresses' },
    { label: 'Collateral floor',      value: '150% ratio min' },
  ...
  return (
    <div className="flex flex-col gap-3">
      <AnimatePresence>
        {phase >= 1 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.35 }}>
            <p className="text-[9px] font-semibold uppercase tracking-[0.18em] px-1 mb-2" style={{ color: 'rgba(255,255,255,0.25)' }}>
              Policy Validation
            </p>
            <div className="space-y-1.5">
              {policies.map((pol, i) => (
                ...
                >
                  ...
                  >
                    <svg width="7" height="7" viewBox="0 0 7 7" fill="none">
                      <path d="M1 3.5L2.5 5L6 2" stroke="#10d98c" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </motion.div>
      ...
      <AnimatePresence>
        {phase >= 2 && (
          <motion.div
            ...
            className="relative overflow-hidden rounded-2xl p-4"
            style={{
              background: 'linear-gradient(135deg, rgba(247,147,26,0.10) 0%, rgba(16,217,140,0.06) 100%)',
              border: '1px solid rgba(247,147,26,0.22)',
            }}
          >
            <div className="flex items-center gap-2.5 mb-3">
              ...
              >
                <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
                  <circle cx="6.5" cy="6.5" r="5" stroke="#F7931A" strokeWidth="1.4" strokeDasharray="4 2" />
                </svg>
              </motion.div>
              <p className="text-[11.5px] font-semibold" style={{ color: '#F7931A' }}>
                {phase < 3 ? 'Executing via Mezo…' : 'Execution complete'}
              </p>
              {phase >= 3 && (
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={spring} className="ml-auto">
                  <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
                    ...
                    <path d="M4.5 7.5L6.5 9.5L11 5" stroke="#10d98c" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </motion.div>
              )}
            </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-[10px]">
                ...
                <span className="font-mono" style={{ color: '#F7931A' }}>500 MUSD</span>
              </div>
              <div className="h-[5px] rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.06)' }}>
                <motion.div
                  ...
                  className="h-full rounded-full"
                  style={{ background: 'linear-gradient(90deg, #F7931A80, #F7931A, #10d98c)' }}
                />
              </div>
            </div>
            ...
            <AnimatePresence>
              {phase >= 3 && (
                <motion.div
                  ...
                  className="overflow-hidden"
                >
                  <div className="mt-3 pt-2.5 border-t flex items-center gap-2" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
...
const STEPS: Step[] = [
  { n: '01', label: 'Connect Treasury',     sublabel: 'BTC wallet read access', color: '#F7931A' },
  { n: '02', label: 'Detect Recurring',     sublabel: 'AI pattern analysis',    color: '#f59e0b' },
  { n: '03', label: 'Build Execution Plan', sublabel: 'Queue & reserve-aware',  color: '#10d98c' },
  { n: '04', label: 'Execute in Policy',    sublabel: 'Auditable on Mezo',      color: '#c084fc' },
];
...
export function ProtocolFlowScene() {
  ...
  return (
    <div className="h-full w-full flex flex-col select-none" style={{ minHeight: '520px' }}>
      {/* Step tabs */}
            ...
            >
              {isActive && (
                <div
                  ...
                  style={{ width: `${progress * 100}%`, background: `linear-gradient(90deg, ${s.color}70, ${s.color})`, transition: 'none' }}
                />
              )}
              <div className="flex items-center gap-1.5">
                {isDone ? (
                  <div className="w-3.5 h-3.5 rounded-full flex-shrink-0 flex items-center justify-center"
                    ...
                    <svg width="7" height="7" viewBox="0 0 7 7" fill="none">
                      <path d="M1 3.5L2.5 5L6 2" stroke="#10d98c" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </div>
                ) : (
                  <span className="text-[9px] font-mono font-600 flex-shrink-0"
      ...
      <div className="px-3 sm:px-4 pt-4 pb-3 flex-shrink-0">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div>
              <p className="text-[15px] font-semibold leading-none" style={{ color: 'rgba(255,255,255,0.9)', letterSpacing: '-0.028em' }}>
                {step.label}
              </p>
      ...
      <div className="flex items-center justify-center gap-2 pb-3 flex-shrink-0">
        {STEPS.map((s, i) => (
          <button
            ...
            style={{ width: i === activeStep ? 20 : 6, height: 6, borderRadius: 3, background: i === activeStep ? s.color : 'rgba(255,255,255,0.1)' }}
          />
        ))}
      </div>
    </div>
